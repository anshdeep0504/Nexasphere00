...
import { AtSign, Heart, MessageCircle, LogOut } from "lucide-react";
...
const Profile = () => {
  ...
  const folowUnfollowHandler = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        // Optionally, refetch profile to update followers
        const updatedProfile = await api.get(`/api/v1/user/profile/${id}`);
  ...
  const messageHandler = () => {
    // You may want to set selected user in redux here if needed
    navigate("/chat");
  };
    ...
    <div className="max-w-3xl mx-auto bg-white dark:bg-[#121212] rounded-xl shadow font-sans p-0 mt-8 text-black dark:text-white">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            ...
            {isLoggedInUserProfile ? (
              ...
              <Button variant="outline" className="rounded-md border px-4 py-1 text-sm font-semibold text-black dark:text-white border-gray-300 dark:border-[#262626]" onClick={folowUnfollowHandler}>Unfollow</Button>
            ) : (
              <Button variant="primary" className="rounded-md px-4 py-1 text-sm font-semibold bg-blue-500 text-white" onClick={folowUnfollowHandler}>Follow</Button>
            )}
            {!isLoggedInUserProfile && (
              <Button variant="outline" className="rounded-md border px-4 py-1 text-sm font-semibold text-black dark:text-white border-gray-300 dark:border-[#262626]" onClick={messageHandler}>Message</Button>
            )}
          </div>
      ...
      <div className="border-t border-gray-200 dark:border-[#262626] px-6 py-4">
        <div className="flex gap-4 mb-4">
          <button
            className={`px-4 py-2 rounded font-semibold transition-colors ${activeTab === 'posts' ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-[#23232b] dark:text-white'}`}
            onClick={() => setActiveTab('posts')}
          >
            Posts
          </button>
          <button
            className={`px-4 py-2 rounded font-semibold transition-colors ${activeTab === 'saved' ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-[#23232b] dark:text-white'}`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
          {(activeTab === 'posts' ? profile?.posts : profile?.bookmarks)?.length === 0 ? (
            ...
            (activeTab === 'posts' ? profile?.posts : profile?.bookmarks)?.map((post) => (
              <div key={post?._id} className="relative group cursor-pointer aspect-square overflow-hidden rounded-md bg-gray-100 dark:bg-[#23232b]" onClick={() => { dispatch(setSelectedPost(post)); setOpen(true); }}>
                <img src={post?.image} alt="postimage" className="object-cover w-full h-full transition-transform duration-200 group-hover:scale-105" />
